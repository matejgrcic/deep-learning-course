...
# definirajte skrivene varijable i pripadajući generator šuma
    ...
    z_sigma = vae_layer(layer_e2, n_hidden_recog_2, n_z, 'z_sigma', act=tf.identity)
    z_log_sigma_sq = tf.log(tf.square(z_sigma))
...
x_reconstr_mean = vae_layer(layer_d2, n_hidden_gener_2, n_input, 'x_mean', act=tf.identity)
...
with tf.name_scope('cost'):
    ...
    cost2 = -0.5 * (1 + z_log_sigma_sq - z_mean * z_mean - z_sigma * z_sigma)
...
bargraph_vis(1, z_mu, [5000, z_mu.shape[1]], 'g', labels)
